...
var saveRooms = [[0, 9, "down"]]; // [level, room, entrace direction]
var currentSave = -1;
            ...
            if (playerY < 376 + 8 && playerY > 376 - 16) { // Check player Y position
                for (var j = 0; j < 3; j++) { // for each shop item
                    if (shopRooms[i][j+4] != -1 && playerX > itemX[j] - 40 && playerX < itemX[j] + 40) {
                        ...
                        if (shopPrices[shopRooms[i][j+4]] < 10) drawImage(NUMBERS[shopPrices[shopRooms[i][j+4]] % 10], itemX[j], 260);
                        else {
                            drawImage(NUMBERS[Math.floor(shopPrices[shopRooms[i][j+4]] / 10)], itemX[j]-9, 260);
